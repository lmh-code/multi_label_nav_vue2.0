    ...
    <div class="search-form-wraper">
      <el-form :model="searchContent" label-position="top">
        ...
        <el-form-item label="供应商">
          <el-select 
            class="search-input" 
            v-model="searchContent.listVendorNo" 
            placeholder="选择供应商" 
            collapse-tags
            clearable
  ...
  export default {
    ...
    methods: {
      ...
      doSearchHandle(firstPage) {
        ...
        let param = {
          ...this.searchContent,
          listVendorNo: this.searchContent.listVendorNo ? [this.searchContent.listVendorNo] : [], 
          pageNum: this.pageNum, 
          pageSize: this.pageSize,
          listGoodsNo: this.productInfo ? [this.productInfo] : null
        }
        if(this.timeType === 0) {
        ...
        this.loading = true
        this.$http.post(warehouseUrl.receiveList, param).then(res => {
          this.loading = false
          if(res && res.code === 0) {
      ...
      doExortHandel() {
        if(this.checkedList && this.checkedList.length) {
          ...
          exportExcel.ExportJsonToExcel({
            ...
            filename: `仓库收货单${dateFormat('yyyy-MM-dd', new Date())}`
          })
      ...
      goDetailPage(row) {
        this.$router.push(`/receiveMgt/${row.paperNo}`)
      },
