...
<script>
  ...
  export default {
    ...
    methods: {
      setDefault(storeNo, dcNo) {
        this.searchContent.dcNo = dcNo
        this.searchContent.storeNo = storeNo
        if(dcNo) {
          this.checkedDialogList = []
          this.getTemplateList(dcNo)
       ...
       * @DateTime 2018-06-21T16:49:26+0800
       ...
       * [changeCurrent 点击分页条]
       * @author liuminghao@benlai.com
       * @DateTime 2018-06-21T16:47:44+0800
        ...
        this.$http.post(reUrl.listStoreGoods, { ...this.searchContent, pageNum: this.currentPage, pageSize: this.pageSize}).then(res => {
          ...
          if(res.code === 0) {
            if(res.data && res.data.list && res.data.list.length) {
              ...
              // if (this.checkedDialogList && this.checkedDialogList.length) {
              //   setTimeout(() => {
              //     this.checkedDialogList.forEach(rowItem => {
      ...
      handleSelect(val, row) {
        let hash = {}
        let checkedListTemp = []
        if(utils.isKeyInArray(row.goodsNo, this.checkedDialogList)) {
      ...
      sureTableItem(param) {
        if(param) {
          ...
          this.$emit("closeAddGoods", dataMesg)
        }else {
          let dataMesg = {
            isShow: false
          }
